...
std::vector<MemoryRegion> ProcessRuntimeUtility::GetProcessMemoryLayout1() {
    ...
    while (!feof(fp)) {
        ...
        if (sscanf(line_buffer,
                   ...
                   &inode,
                   &path_index) < 7) {
            LOGD("/proc/self/maps parse failed!");
            fclose(fp);
            return ProcessMemoryLayout;
        }
...
std::vector<RuntimeModule> ProcessRuntimeUtility::GetProcessModuleMap1() {
...
RuntimeModule ProcessRuntimeUtility::GetProcessModule1(const char *name) {
    std::vector<RuntimeModule> ProcessModuleMap = GetProcessModuleMap1();
