...
int elf_ctx_init1(elf_ctx_t *ctx, void *header_) {
...
void *elf_ctx_iterate_symbol_table1(elf_ctx_t *ctx, const char *symbol_name) {
  void *result = NULL;
...
void *resolve_elf_internal_symbol1(const char *library_name, const char *symbol_name) {
  void *result = NULL;

  if (library_name) {
    RuntimeModule module = ProcessRuntimeUtility::GetProcessModule1(library_name);
    ...
    if (file_mem) {
      elf_ctx_init1(&ctx, file_mem);
      result = elf_ctx_iterate_symbol_table1(&ctx, symbol_name);
    }
  ...
  if (!result) {
    std::vector<RuntimeModule> ProcessModuleMap = ProcessRuntimeUtility::GetProcessModuleMap1();
...
PUBLIC void *DobbySymbolResolver(const char *image_name, const char *symbol_name_pattern) {
  ...
  result = resolve_elf_internal_symbol1(image_name, symbol_name_pattern);
  return result;
}